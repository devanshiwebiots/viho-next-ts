...
// const SidebarMenuDropDown: React.FC<SidebarMenuDropDownType> = ({ setLinkActive, linkActive, items, handleOpen, active, level, setActive }) => {
//   const { t } = useTranslation("common");
//   const router = useRouter();
//   const pathname = useSearchParams().get("layout");

//   const handleClick = (event: React.MouseEvent<HTMLAnchorElement, MouseEvent>, element: any) => {
...
//   return items?.map((element, index) => (
//     <ListGroupItem className='dropdown' key={index}>
//       <Link href={element?.path ? `${element?.path}` : ""} onClick={(event) => handleClick(event, element)} className={`${(element.pathSlice && active?.includes(element.pathSlice)) || linkActive == element.path?.split("/")[element.path.split("/").length - 1] ? "active" : ""} ${level == 0 ? `nav-link menu-title` : ""}`}>
//         {level === 0 ? (
//           <>
//             {element.icon}
//             <span>{t(element.title)}</span>
