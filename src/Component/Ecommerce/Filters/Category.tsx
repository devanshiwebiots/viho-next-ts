...
import { deleteBrandState, deleteCategoryState, filterBrandState, filterCategoryState, setColorState, setPrizeRange } from "@/Redux/Slices/E-comerceSlice";
    ...
    <Fragment>
          ...
          {Category().map((item, i) => {
            return (
              <Label className='d-block' key={i}>
                <Input className='radio_animated' defaultChecked={filterValue.category.includes(item) ? true : false} type='checkbox' name='name' value={item} onClick={filterCategory} />
                {item}
              </Label>
            );
          })}
      ...
      <div className='product-filter slider-product'>
        ...
        <div className='color-selector'>
          <ListGroup className=' d-flex flex-row gap-1'>
